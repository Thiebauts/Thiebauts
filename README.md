...
The repository ships with a runnable Kaggle notebook, a ~125-page LaTeX report covering full methodology and ablations, the hackathon writeup, and aggregate MIMIC-IV-ED supplementary results (figures + JSON/CSV summaries only, with no patient-level data, in line with PhysioNet's data-use agreement).
